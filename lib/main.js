...
var createTag = function(src, asset, attributes, version, forcedTag) {
  // Cachebusting
  version = version || '';
  // custom tag
  var tag = forcedTag;
  ...
  switch(mime.lookup(asset.split('?')[0])) {
    ...
    case 'text/xml':
      ...
      attributes.content = src + asset + version;
      return '<meta ' + renderAttributes(attributes) + ' />';
    case 'text/css':
...
var renderTag = function(options, assets, attributes, forcedTag) {
  // Set attributes
  ...
  if (options.production) {
    ...
    if (typeof assets === 'object') {
      ...
      return createTag(src, "/" + name, attributes, null, forcedTag);
    } else {
    ...
    if (typeof assets === 'object') {
      for (var i=0; i<assets.length; i+=1) {
        buf.push(createTag(src, assets[i], attributes, version, forcedTag));
        if ( (i + 1) === assets.length) return buf.join("\n");
      }
    } else if (typeof assets === 'string') {
      return createTag(src, assets, attributes, version, forcedTag);
    } else {
...
var CDN = function(app, options, callback) {
      ...
      walker.on('end', function() {
        ...
        for (var i=0; i<results.length; i+=1) {
          ...
          if (attributeIndex !== -1) {
            ...
            var forceIndex = results[i].indexOf('}');

            // if we have the forcedTag param
            if (forceIndex !== -1 && results[i][forceIndex + 1] === ',') {
              results[i] = _(results[i]).splice(forceIndex + 2, 0, '"forcedTag": ');
            }
          }
  ...
  return function(req, res) {
    return function(assets, attributes, forcedTag) {
      if (typeof assets === 'undefined') throwError('assets undefined');
      return renderTag(options, assets, attributes, forcedTag);
    };
  };
